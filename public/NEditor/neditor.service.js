...
UE.Editor.prototype.getActionUrl = function(action) {
    ...
    if (action == 'uploadimage' || action == 'uploadscrawl') {
        return 'http://hc.6636fu.com/BusinessAfter/UploadImg/?newfile=richtext';
...
window.UEDITOR_CONFIG['imageUploadService'] = function(context, editor) {
    return {
        ...
        getResponseSuccess: function(res) {
            console.log(context)
            let imgUrl=res.data.returnurl
            res.data.returnurl='http://hc.6636fu.com'+imgUrl
            return res.code =='00000';
        },
        /* 指定上传接口返回的response中图片路径的字段，默认为 url
         * 如果图片路径字段不是res的属性，可以写成 对象.属性 的方式，例如：data.url 
         * */
        imageSrcField: 'data.returnurl'
    }
};
